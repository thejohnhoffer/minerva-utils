...
def find_group_channels(exhibit):
    ex = load_json(exhibit)
    group = ex.get(
        'FirstGroup', ex['Groups'][0]["Name"]
    )
    chan_paths = {
...
def find_group_tiles(root, exhibit):
    channel_list = find_group_channels(exhibit)
...
def merge_tiles_and_save_image(root, tiles):
    ...
    # Save image to multilpe locations
...
if __name__ == '__main__':
    ...
    tiles = find_group_tiles(args.root, args.json)
